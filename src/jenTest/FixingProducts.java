...
public class FixingProducts{
	...
	@Test(enabled=true,priority=1)
	  public void FixingsProductsPage() throws IOException, InterruptedException {
		  File file = new File("C:\\Selenium\\jenkindemo\\src\\objectRepositry\\Products_PageObjects");
		  FileInputStream input = new FileInputStream(file);
		  Properties prop = new Properties();
		  prop.load(input);
		  dr.navigate().to(prop.getProperty("FixingsProductsPage"));
		  ...
		  for(int i=1;i<=t;i++){
			  ...
			  for(int n=1; n<=Subtotal; n++){
				  ...
				  String ProEnd[]={"65mm Polynails (white)", "300mm White Cable Ties", "30mm Plated Screws"};
